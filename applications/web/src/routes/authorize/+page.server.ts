...
export const load: PageServerLoad = async ({ url, locals }) => {
  if (!locals.user) {
    ...
    // Redirect to the BETTER_AUTH_URL origin so that OAuth state cookies are
    // set on the same domain that receives Google's callback. Without this,
    // requests arriving via a Cloudflare tunnel would set cookies for the
    // tunnel domain while Google redirects to localhost → state_mismatch.
