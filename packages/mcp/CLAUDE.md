...
Prompts read Markdown templates via `node:fs/promises` using `import.meta.resolve()` to locate the file path. See `adapt-lesson.ts` or `setup-classroom-profile.ts` for the pattern.
