...
export interface AdaptationSummary {
  ...
  depthMode: 'quick-scan' | 'standard' | 'deep-dive';
...
export const learningProfiles = pgTable('learning_profiles', {
  ...
  userId: uuid('user_id')
    ...
    .unique()
    .references(() => neonAuthUsers.id),
  needs: jsonb('needs').$type<LearningNeed[]>().notNull().default([]),
  teachingContext: jsonb('teaching_context').$type<TeachingContext>().notNull().default({}),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
});

export const adaptedLessons = pgTable(
  'adapted_lessons',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    userId: uuid('user_id')
    ...
    adaptationSummary: jsonb('adaptation_summary').$type<AdaptationSummary>().notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [index('idx_adapted_lessons_user_created').on(table.userId, table.createdAt)],
);

export const adaptationReflections = pgTable(
  'adaptation_reflections',
  {
    ...
    userId: uuid('user_id')
      .notNull()
      .references(() => neonAuthUsers.id),
    adaptedLessonId: uuid('adapted_lesson_id')
      .notNull()
      .references(() => adaptedLessons.id),
    reflectionData: jsonb('reflection_data').$type<ReflectionData>().notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
    unique().on(table.userId, table.adaptedLessonId),
    index('idx_adaptation_reflections_adapted_lesson').on(table.adaptedLessonId),
  ],
);
